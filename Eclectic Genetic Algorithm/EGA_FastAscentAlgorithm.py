...
delimiter = input("Input separator's type (, or tab): ")

### obtener parametros del usuario: 
n_individuals = int(input("Give the numbers of individuals (int): "))
cross_probability = float(input("Give me the cross probability [0,1]: "))
mutation_probability = float(input("Give me the mutation probability [0,1]: "))
max_generations = int(input("Give me the number of generations (int): "))
max_degree = int(input("Give me the maximum allowed degree (int): "))
highest_monomial_degree = int(input("Give me the highest monomial's degree (int): "))
error = input("Give the error to minimize (rms/mmx): ")

# impresion de los resultados
### calcular el numero de terminos y de variables independientes
data = readData(path,delimiter=delimiter)
n_terms = findNumberTerms(path,delimiter=delimiter);
n_ind_variables = data.shape[1]-1
print('\nTerms number: ',n_terms)
print('Independent variables number: ',n_ind_variables)

_,_,_ = EGA_CV(data,n_individuals,cross_probability,mutation_probability,
           max_generations,n_terms,error=error,max_degree=max_degree,max_monomial_degree=highest_monomial_degree,
           distribution='normal',train_pp=0.8,randomly_split=True,
            stabilize_data=True,stabilize_factor=1/1e6,
           plot_res_fitness=True,plot_best_results=True,plot_sort=False,
